...
Dataset: peso_kg | cilindrada_L | potencia_hp | anio  →  consumo_km_litro (Y)
...
ruta = "/home/bm-003/MachineLearning/caso1_combustible.csv"
datos = []

with open(ruta, "r", encoding="utf-8") as f:
    encabezado = f.readline().strip().split(",")

    idx_x1 = encabezado.index("peso_kg")
    idx_x2 = encabezado.index("cilindrada_L")
    idx_x3 = encabezado.index("potencia_hp")
    idx_x4 = encabezado.index("anio")
    idx_y  = encabezado.index("consumo_km_litro")

    for linea in f:
        partes = linea.strip().split(",")
        x1 = float(partes[idx_x1])   # peso_kg
        x2 = float(partes[idx_x2])   # cilindrada_L
        x3 = float(partes[idx_x3])   # potencia_hp
        x4 = float(partes[idx_x4])   # anio
        y  = float(partes[idx_y])    # consumo_km_litro (target)
...
def gradient_descent_multivariable(X, y, w_init, lr, iteraciones,
    ...
    if verbose:
        ...
        etiquetas = ["w0(bias)", "w1(peso)", "w2(cilin)", "w3(potenc)", "w4(anio)"]
    ...
    if verbose:
        ...
        print(f"\n Resultado {nombre}:")
        etiquetas = ["w0(bias)", "w1(peso_kg)", "w2(cilindrada_L)",
                     "w3(potencia_hp)", "w4(anio)"]
...
def barras_w_inicial_vs_final(w_inicial, w_final):
    etiquetas = ["w0(bias)", "w1(peso)", "w2(cilin)", "w3(pot)", "w4(anio)"]
...
if __name__ == "__main__":
    ...
    NUM_FEATURES = 4   # peso_kg | cilindrada_L | potencia_hp | anio

    print("\n" + "#"*75)
    print("  GD MULTIVARIABLE | Z-score | 3 Inicializaciones de Pesos")
    print("  Features: peso_kg | cilindrada_L | potencia_hp | anio -> consumo_km_litro")
    print("#"*75)
    ...
    nombres_features = ["peso_kg", "cilindrada_L", "potencia_hp", "anio"]
    ...
    print(f"   {'consumo_km_litro':<15}  (NO normalizado)")
    ...
    etiquetas = ["w0(bias)", "w1(peso_kg)", "w2(cilindrada_L)", "w3(potencia_hp)", "w4(anio)"]
    ...
    # -----------------------------------------------------------------------
    historico = [3.533500, -0.769780, -0.066870, 0.586050, -0.208580]
    ...
    print(fmt.format("Metodo", "w0", "w1(peso)", "w2(cil)", "w3(pot)", "w4(anio)", "MSE"))
