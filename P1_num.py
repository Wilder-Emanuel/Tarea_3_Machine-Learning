...
def gradient_descent_multivariable(X, y, lr=0.01, epocas=500, epsilon=1e-6):
    ...
    # 1. INICIALIZACIÓN MANUAL DE PESOS (w0, w1, w2, w3, w4)
    valores_manuales = [3.53350, -0.76978, -0.06687, 0.58605, -0.20858]
    w = np.array(valores_manuales)
...
if __name__ == "__main__":
    ...
    df = pd.read_csv('caso1_combustible.csv')

    features = ['peso_kg', 'cilindrada_L', 'potencia_hp', 'anio']
    target = 'consumo_km_litro'
    ...
    for lr_test, color in zip(learning_rates, colores):
        ...
        w_final, historial = gradient_descent_multivariable(
            X_final, y, lr=lr_test, epocas=1000
        )
